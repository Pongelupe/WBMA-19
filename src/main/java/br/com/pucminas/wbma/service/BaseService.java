...
import br.com.pucminas.wbma.exceptions.RequestException;
...
public class BaseService {
	 ...
	 * timeout. The timeout was intended for high delayed operations
	 * 
	 * 
	 * @param auth
	 */
	protected void setTokenOnRequestHeader(Auth auth) {
		client = client.newBuilder().readTimeout(60, TimeUnit.SECONDS).addInterceptor(chain -> {
			...
			Request request = original.newBuilder().header("User-Agent", "PUCMINAS")
					.header("Authorization", auth.getTokenType() + " " + auth.getTokenAccess())
